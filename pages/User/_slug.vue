            ...
            <div class="md:flex items-center">
                ...
                <div class="flex-1 md:ml-4 space-y-2">
                    <div class="flex justify-center md:justify-end space-x-2">
                        <button @click='contact' v-if='$auth.loggedIn && $auth.user.id != user.id' class="rounded p-2 text-xs text-gray-50 bg-blue-400">Liên hệ</button>
                        <nuxt-link to='/settings/profile' v-if='$auth.loggedIn && $auth.user.id == user.id' class="block rounded p-2 text-xs text-gray-50 bg-blue-400">Chỉnh sửa hồ sơ</nuxt-link>
                    </div>
                    <div class="">
...
export default {
    async asyncData({ params,$axios }) {
      ...
      const user = await $axios.$get(`api/user/overview/${slug}`)
      user.numberphone = 'Đã ẩn' 
      user.email ='Đã ẩn'
      return { user }
    },
    methods:{
