<template>
  <div class="mx-auto">
    ...
    <div v-else class="p-4 px-2 md:container mx-auto">
      <FormInput class="block pb-4 shadow-md rounded-xl mb-8" title='Cập nhật thông tin' textBtn='Cập nhật thông tin' @submit='onUpdate'>
        <template>
          ...
          <ValidationProvider ref="attachedFile" name="File đính kèm" rules='ext:jpg,png,doc,docx,pdf,zip,rar|size:2000' v-slot="{ errors }">
...
export default {
  ...
  methods:{
    onUpdate(){
      ...
      this.$axios.$post("api/user/project/update",formData)
      .then(()=>{this.$router.replace('/user/project');this.$store.commit('view/closeBar')})
